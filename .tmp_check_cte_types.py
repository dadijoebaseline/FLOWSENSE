...
billing_with_spatial AS (
    SELECT
        ...
        b.billamount,
        COALESCE(b.sync_year::text, '2026') AS sync_year,
        COALESCE(b.sync_month, 'APRIL') AS sync_month
    FROM latest_qgis_main q
    LEFT JOIN latest_tcwd_database d ON q.acct = d.acct
...
FROM billing_with_spatial
LIMIT 1
""")
print('billing_with_spatial', cur.fetchone())
cur.execute("""
WITH latest_tcwd_database AS (
    SELECT DISTINCT ON (TRIM(BOTH FROM accountnumber))
        TRIM(BOTH FROM accountnumber) AS acct,
